...
function buildResearchPrompt(input: CaseLawSearchInput): string {
...
Return ONLY a valid JSON object in this exact shape — no markdown, no preamble, no explanation outside the JSON:
{
  ...
  const modeContext: Record<SearchMode, string> = {
...
class CaseLawService {
  ...
  async search(lawyerId: string, input: CaseLawSearchInput) {
    ...
    if (matterId) {
      ...
      if (!matter) throw new AppError('Matter not found.', 404, 'MATTER_NOT_FOUND');
    }
    ...
    if (matterId) {
      ...
      if (memory) workspaceMemoryPayload = workspaceMemoryService.buildPayload(memory);
    }
    ...
    try {
      ...
      throw new AppError(
        'The AI research service encountered an error. Please try again.',
        502,
        'AI_SERVICE_ERROR'
      );
    }
  ...
  async saveSelected(lawyerId: string, input: {
    matterId: string;
    query: string;
    searchMode?: string;
    jurisdiction?: string;
...
${caseBlocks}`;
  ...
  private parseStructuredResponse(text: string): {
    ...
    try {
      ...
      const cases: CaseCitation[] = (parsed.cases ?? []).map(
        (c: any, i: number): CaseCitation => ({
          index: i + 1,
  ...
  private async saveResearchToMatter(
    lawyerId: string,
    matterId: string,
    query: string,
    content: string
