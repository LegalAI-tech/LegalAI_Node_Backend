...
class MatterEventService {
  ...
  async createEvent(lawyerId: string, matterId: string, input: CreateEventInput) {
  ...
  async listEvents(lawyerId: string, matterId: string, query: ListEventsQuery) {
    await this.verifyMatterOwnership(lawyerId, matterId);
    ...
    if (isDeadline !== undefined) where.isDeadline = isDeadline;
    if (upcoming) {
  ...
  async getEvent(lawyerId: string, matterId: string, eventId: string) {
    await this.verifyMatterOwnership(lawyerId, matterId);
  ...
  async updateEvent(
    ...
    let cleanReminderDays: number[] | undefined;
  ...
  async completeEvent(lawyerId: string, matterId: string, eventId: string) {
    await this.verifyMatterOwnership(lawyerId, matterId);
  ...
  async deleteEvent(lawyerId: string, matterId: string, eventId: string) {
    await this.verifyMatterOwnership(lawyerId, matterId);
  ...
  async getUpcomingDeadlines(lawyerId: string, daysAhead = 30) {
    ...
    return events.map(e => ({
      ...this.formatEvent(e),
      matter: (e as any).matter,
    }));
  }

  private async verifyMatterOwnership(lawyerId: string, matterId: string) {
