...
class MatterDocumentService {
  ...
  async uploadDocument(
    ...
    try {
      const indexResult = await pythonBackendService.agentUploadAndChat(
        file.buffer,
        file.originalname,
        'Please analyse and index this legal document. Provide a brief summary of its key contents.',
      );
  ...
  async listDocuments(lawyerId: string, matterId: string) {
    const matter = await prisma.matter.findFirst({
      where: { id: matterId, lawyerId },
      select: { id: true },
    });
  ...
  async deleteDocument(lawyerId: string, matterId: string, docId: string) {
    const matter = await prisma.matter.findFirst({
      where: { id: matterId, lawyerId },
      select: { id: true },
    });
    ...
    if (memory && Array.isArray(memory.documentIndex)) {
      const filtered = (memory.documentIndex as any[]).filter(d => d.id !== docId);
  ...
  private resolveDocType(type?: string): 'UPLOADED' | 'GENERATED' | 'ORDER' | 'EVIDENCE' | 'CORRESPONDENCE' {
