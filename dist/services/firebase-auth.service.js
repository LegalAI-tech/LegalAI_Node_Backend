...
import { generateCitizenTokens, generateTwoFaTempToken, } from '../utils/jwt.utils.js';
import otpService from '../services/otp.service.js';
import notificationService from '../services/notification.service.js';
import encryptionService from '../services/encryption.service.js';
import { validateLawyerRegistration } from '../utils/validator.js';
class FirebaseAuthService {
    async citizenGoogleLogin(idToken) {
        const decoded = await verifyFirebaseToken(idToken);
        const { uid, email, name, picture } = decoded;
        if (!email) {
            throw new AppError('Google account does not have an email address.', 400, 'NO_EMAIL_FROM_GOOGLE');
        }
    ...
    async lawyerGoogleLogin(idToken, additionalData) {
        const decoded = await verifyFirebaseToken(idToken);
        const { uid, email, name, picture } = decoded;
        if (!email) {
            throw new AppError('Google account does not have an email address.', 400, 'NO_EMAIL_FROM_GOOGLE');
        ...
        if (!lawyer) {
            const { phone = '0000000000', barNumber = 'PENDING', barCouncilState = 'PENDING', practiceAreas = [], yearsOfExperience, } = additionalData || {};
        ...
        if (lawyer.isLocked) {
            throw new AppError(`Account is locked. Reason: ${lawyer.lockReason || 'Security hold'}`, 403, 'ACCOUNT_LOCKED');
        }
