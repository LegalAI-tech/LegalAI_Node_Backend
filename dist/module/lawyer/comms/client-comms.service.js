...
function buildCommsPrompt(input, matterTitle, workspaceContext) {
...
class ClientCommsService {
    async generate(lawyerId, input) {
        ...
        try {
            ...
            throw new AppError('The AI service encountered an error generating communications. Please try again.', 502, 'AI_SERVICE_ERROR');
        }
    ...
    buildContextString(memory) {
        if (!memory)
            return '';
        const parts = [];
        if (memory.partySummary)
            parts.push(`Parties: ${memory.partySummary}`);
        if (memory.legalIssues)
            parts.push(`Legal issues: ${memory.legalIssues}`);
        if (memory.aiSummary)
            parts.push(`Case summary: ${memory.aiSummary}`);
        if (memory.keyDates && Array.isArray(memory.keyDates) && memory.keyDates.length > 0) {
            const dates = memory.keyDates
                .slice(0, 3)
    ...
    parseFormats(text, format) {
        ...
        if (!hasAnyContent && text) {
            if (format === 'whatsapp')
                ...
                result.emailBody = text;
            else if (format === 'voice_note')
                result.voiceNote = text;
        }
        return result;
    }
}
export default new ClientCommsService();
//# sourceMappingURL=client-comms.service.js.map